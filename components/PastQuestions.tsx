...
import { FaChevronLeft, FaChevronRight, FaCheck, FaTimes, FaQuestionCircle, FaExclamationTriangle } from "react-icons/fa";
...
const PastQuestions = ({ courseCode, onClose }: PastQuestionsProps) => {
  ...
  if (questions.length === 0) return <div className="text-center text-red-500">No questions available for {courseCode}.</div>;
    ...
    <AnimatePresence>
      {!showResults ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="bg-white rounded-lg p-6 w-full max-w-2xl">
            ...
            <div className="flex justify-between mb-4">
              <button onClick={handlePrevious} disabled={currentIndex === 0} className="flex items-center text-green hover:text-deepGreen">
                <FaChevronLeft className="mr-2" /> Previous
              </button>
              <button onClick={handleNext} disabled={currentIndex === questions.length - 1} className="flex items-center text-green hover:text-deepGreen">
            ...
            <div className="flex justify-between">
              ...
              <button onClick={handleSubmit} className="bg-green text-white px-4 py-2 rounded-lg hover:bg-deepGreen">Submit</button>
            </div>
            {showExplanation && <div className="mt-4 p-4 bg-beige rounded-lg"><p>{questions[currentIndex]?.explanation}</p></div>}
          </motion.div>
        </motion.div>
      ) : (
        <motion.div className="fixed inset-0 bg-white flex flex-col items-center justify-center p-4">
          ...
          <button onClick={onClose} className="mt-4 bg-green text-white px-4 py-2 rounded-lg hover:bg-deepGreen">Close</button>
        </motion.div>
      )}
    </AnimatePresence>
  );
